var todoApp=angular.module('starter.controllers', ['LocalStorageModule']);
todoApp.run(function($rootScope,localStorageService){
  if(!localStorageService.get('userName')){
   localStorageService.set('userName',"");
  }
});
todoApp.filter('truster',function($sce){
  return function(text){
    return $sce.trustAsResourceUrl(text);
  }
});
// todoApp.config(function ($compileProvider) {
//   $compileProvider.imgSrcSanitizationWhitelist('.jpg|.png');
// });
  //主页
todoApp .controller('HomeCtrl', function($scope,HomeGoodlistRow,$rootScope,$ionicSideMenuDelegate,locals,$http,$ionicSlideBoxDelegate,shareData,$window) {
  ...
  //课程列表
  .controller('LessonlistCtrl', function($scope, courseLists,$http,$rootScope,$timeout,shareData,localStorageService) {

    //请求专业分类子级内容数据
    // $http.post($rootScope.URLAdmin+'/Handler/OfflineCourseHandler.ashx?action=getcategory','')
    //   .success(function(response){
    //     console.log(response.data);
    //     localStorageService.set('courseListBtns',response.data);
  ...
  //我的课程
  .controller('MycourseCtrl', function($scope, courseFirst, courseSecond,$http,$rootScope,localStorageService) {
    //先设置让其显示“请登录”
    $scope.dl_tf=true;

    //我的课程数据 请求
    $http.get($rootScope.URLAdmin+'/Handler/OnCourseHandler.ashx?action=mycourse&userName='+localStorageService.get('userName'),'')
      .success(function(response){
        $scope.dl_tf=false;
        console.log(response);
        $scope.itemFir = response.data;
        //收藏课程数据
        $http.get($rootScope.URLAdmin+'/Handler/OnCourseHandler.ashx?action=mycollection&userName='+localStorageService.get('userName'),'')
          .success(function(response){
            //console.log(response.data);
  ...
  //个人中心登录
  .controller('PersonalCtrl', function($scope,$http,$rootScope,localStorageService) {

    //防止页面调试过程中的页面切换导致下部tabs隐藏了，无法切换，这里设置切换到登录页的时候，显示tabs。
    $rootScope.hideTabs = false;
    //console.log($rootScope.userStudent);
    //是否登录请求，如果已登录，直接跳转到个人信息页
    var isLoginUser={//o.hasOwnProperty("x")
      userName:localStorageService.get('userName')
    };
    $http.post($rootScope.URLAdmin+'/Handler/UserHandler.ashx?action=isLogin',isLoginUser)
      .success(function(response){
        //console.log(response);
        if(response.success){
        ...
        $http.post($rootScope.URLAdmin+'/Handler/UserHandler.ashx?action=login',myData)
          .success(function(response){
            ...
            if(response.err){
              ...
              localStorageService.set('userName',$scope.loginuser.name);
              window.location="#/tab/information"
            }
  ...
  //个人信息
  .controller('InformationCtrl', function($scope,$http,$rootScope,localStorageService) {

    //获取个人信息isLogin
    $http.get($rootScope.URLAdmin+"/Handler/OnCourseHandler.ashx?action=returnuserinfo&userName="+localStorageService.get('userName'),'')
      .success(function(response){
        console.log(response)
    ...
    $scope.quit=function(){
      $http.post($rootScope.URLAdmin+"/Handler/UserHandler.ashx?action=quit",quitUser)
        .success(function(response){
          //console.log(response)
          if(response.success){
            localStorageService.set('userName','');
            localStorageService.set('courseListBtns','');
          }
          window.location="#/tab/personal"
        })
    }
  ...
  //学习
  .controller('StudyCtrl', function($scope,$stateParams,studyList,pingjiaList,$ionicModal,$http,$rootScope,$ionicPopup,localStorageService,$sce ) {
    ...
    $http.post($rootScope.URLAdmin+'/Handler/UserHandler.ashx?action=isLogin',isLoginUser)
      .success(function(response){
        if(response.success==="已登录"){
          /*已登录执行的代码*/
          $http.post($rootScope.URLAdmin+"/Handler/OnCourseHandler.ashx?action=learnshow", isLoginUser)
            .success(function(response) {
              console.log('6666')
              ...
              try{
                ...
                $scope.pcTrue = true;
                var urlStr = response.data.CDlist[0].Vlist[0].Vurl;
                while(urlStr.indexOf('\\')!=-1){
                  urlStr=urlStr.replace('\\','/');
                }
                //console.log(urlStr);
                $scope.Vurl = $rootScope.URLAdmin +'/'+urlStr;
                //$scope.Vurl = 'http://127.0.0.1:3000/temporary/video/upload_715e45f27a6b6d732f284b86ca728b85.mp4';
                //console.log(  $scope.Vurl +'999999');
              }
  ...
  //学习页底部导航的控制器
  .controller('studyFooterCtrl',function($scope,$http,$stateParams,$rootScope,localStorageService){
  ...
  //学习页评价控制器
  .controller('TaskCtrl', function($scope,$ionicPopup,$http,$rootScope,localStorageService) {
