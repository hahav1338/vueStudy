...
//登录请求,添加系统人员
router.post('/Handler/AdminLoginAndRegHandler.ashx', function (req, res) {
...
//管理员列表(show,delete)
router.get('/Handler/AdminHandler.ashx',function(req,res){
...
//管理员列表update
router.post('/Handler/AdminHandler.ashx',function(req,res){
...
//获取权限，获取课程列表等
router.get('/Handler/CourseHandler.ashx',function(req,res){
    if(req.query.action=='getpower'){
        ...
        req.query.action = 'find';
        //查询对应ID的目录数据条
        handler(req, res, "directoryList",{ID:parseInt(req.query.ID)} ,function(data){
            //查询该目录对应的课程数据条
            handler(req, res, "productList",{_id:data[0].CDid},function(result){

                //查询视频列表获得要处理的视频数据集
                handler(req, res, "videoList",{Vmosaic:{$regex:'.*'+data[0]._id+data[0].CDid+'.*'}} ,function(da){
                    //var dirIDProName = data[0].CDid+result[0]._id;
                    //迭代处理videoList的Vstate和Vmosaic
                    delDirVideo(req, res, data[0]._id,result[0]._id,result[0].Cname,da,function(){
                        req.query.action = 'find';
                        ...
                        handler(req, res, "directoryList",{ID:parseInt(req.query.ID)},function(data){
                            ...
                            if(data.length==0){
                                ...
                                //删除操作
                                handler(req, res, "directoryList", {ID:parseInt(req.query.ID)},function(da){
                                    console.log(da);
                                    if(da.result.n==0){
                                        res.end('{"err":"删除失败"}');
                                    }else{
                                        ...
                                        handler(req, res, "directoryList",{ID:{$gt:parseInt(req.query.ID)}},function(daa){
                                            if(daa.length==0){
                                                ...
                                                handler(req, res, "directoryList",{CDid:data[0].CDid},function(dat){
                                                    var obj = {
                                                        data:{
                                                            list:dat,
                                                            success:"删除成功"
        ...
        req.query.action='find';
        //查询目录列表列表获取当前目录的_id
        handler(req, res, "directoryList", {ID:parseInt(req.query.Did)},function(set){
            //查询课程列表获取对应课程名
            handler(req, res, "productList", {ID:parseInt(req.query.courseId)},function(data){
                if(data.length==0){
                    res.end('{"err":"抱歉，视频添加失败"}');
                }else{
                    //查询视频列表获取绑定课程字段Vmosaic，用以拼接

                    handler(req, res, "videoList", {ID:parseInt(req.query.Vid),Vmosaic:{$regex:'(?!.*'+set._id+data[0]._id+'^.*$)'}},function(dat){
        ...
        handler(req, res, "directoryList", {ID:parseInt(req.query.Did)},function(set){
            ...
            handler(req, res, "productList", {ID:parseInt(req.query.courseId)},function(data){
                if(data.length==0){
                    ...
                    handler(req, res, "videoList", {ID:parseInt(req.query.Vid)},function(dat){
                        if(dat.length==0){
...
router.post('/Handler/CourseHandler.ashx',function(req,res){
    ...
    if(req.query.action=='show'){
        ...
        handler(req, res, "productList",{"ID":parseInt(req.body.CDid)},function(set){
            ...
            handler(req, res, "directoryList",null,function(arr){
                var objDirectory = {
                    ...
                    CDid:set[0]._id,
                    CourseNumber:0,
                    ID:req.body.ID?req.body.ID:arr.length+1
                }
                req.query.action = 'add';
                if( objDirectory.CDName&& objDirectory.CDid){
                    ...
                    handler(req, res, "directoryList",objDirectory,function(data){
                        if(data.ops.length==0){
                            ...
                            handler(req, res, "directoryList",{CDid:set[0]._id},function(data){
                                if(data.length==0){
                                    res.end('{"err":"抱歉，目录添加失败"}');
                                }else{
                                    var obj = {
        ...
        if(flagInt===0){
            req.query.action='find';
            //获取该条课程信息
            handler(req, res, "productList",{ID:parseInt(req.body.ID)},function(set){
                if(set.length!==0){
                    flagInt++;
                    //迭代处理该课程对应的目录---删除目录并处理与之绑定的视屏
                    delProDir(req, res,set[0]._id,function(){
                        req.query.action='delete';
                        //删除对应ID的document
                        handler(req, res, "productList",{ID:parseInt(req.body.ID)},function(data){
                            if(data.result.n==0){
                                res.end('{"err":"删除失败"}');
                            }else{
                                req.query.action = 'find';
...
//图片上传的处理逻辑
router.post('/Handler/UpLoadPicHandler.ashx',function(req,res){
...
//增加或更新数据库中的视频列表信息
router.post('/Handler/VideoHandler.ashx',function(req,res){
...
router.get('/Handler/VideoHandler.ashx',function(req,res){
    //删除视频列表中的视频
    if(req.query.action=='delete'){
        ...
        //根据ID获取对应的目录document
        handler(req, res, "directoryList",{ID:parseInt(req.query.DirID)} ,function(data){
            if(data.length==0){
                var obj = {
                    data:{
                        list:[]
                ...
                handler(req, res, "productList",{_id:new ObjectID(data[0].CDid)},function(result){
        ...
        handler(req, res, "videoList", null,function(arr){
            if(isNullObj(selector)){
                selector={ID:{$gt:arr.length-(parseInt(req.query.pageStart)*3-3)-3,$lte:arr.length-(parseInt(req.query.pageStart)*3-3)}};
            }
            //分页查询当前三条数据
            handler(req, res, "videoList",selector ,function(data){
                if(data.length==0){
...
//上传视频
router.post('/Handler/UpLoadVideoHandler.ashx',function(req,res){
    var form = new formidable.IncomingForm();
    form.encoding = 'utf-8';
